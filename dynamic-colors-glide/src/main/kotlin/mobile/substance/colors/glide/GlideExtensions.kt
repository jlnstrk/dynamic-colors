...
inline fun RequestBuilder<DynamicColorsWrapper<UIColorPackage>>.mode(mode: Int)
    ...
    return apply(
        RequestOptions()
            .set(
                Option.memory(DynamicColorsTranscoder.UI_COLORS_MODE_PRIMARY),
                mode and DynamicColors.MASK_PRIMARY
            )
            .set(
                Option.memory(DynamicColorsTranscoder.UI_COLORS_MODE_ACCENT),
...
inline fun <reified CP : ColorPackage> RequestBuilder<DynamicColorsWrapper<CP>>.into(
    imageView: ImageView,
    onColorsExtracted: OnColorsExtractedListener<CP>
): DynamicColorsTarget<CP> {
    val target = when (CP::class.java) {
        UIColorPackage::class.java -> DynamicColorsTarget.UIImpl(
            imageView,
            onColorsExtracted as OnColorsExtractedListener<UIColorPackage>
        )
        DominantColorPackage::class.java -> DynamicColorsTarget.DominantImpl(
            imageView,
            onColorsExtracted as OnColorsExtractedListener<DominantColorPackage>
        )
        else -> throw IllegalArgumentException(
            "Type parameter 'CP' must be either UIColorPackage " +
